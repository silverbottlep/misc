...
string outdir = "/home/eunbyung/Works/data/ucf101_opt_new/";
...
int main(int argc, char** argv )
{
	...
	for( std::string fname; getline(filelist, fname); )
	{
		...
		for (int i=0; i<N; i++) {
			int c_len = fname.length() - 8;	
			char c_name[128]={0};
			char str[6]={0};
			int ncopy = fname.copy(c_name,c_len,0);
			sprintf(str, "%04u", i+1);
			string f1 = dir+c_name+"/"+fname+"_f"+str+".jpg";
			cout << f1 << endl;
...
//			int ncopy = fname.copy(c_name,c_len,0);
//			sprintf(str, "%04u", i+1);	
//			string f = outdir+c_name+"/"+fname+"_f"+str+"_opt.jpg";
...
//			vconcat(flow_x[i],flow_y[i],flow[i]);
//			imwrite(f, flow_x_8u[i]);
//
